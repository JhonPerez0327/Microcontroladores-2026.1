...
INIT:
        ; OSCCON:
        ; ((OSCCON) and 0FFh), 6, a:((OSCCON) and 0FFh), 4, a = 110 -> 4 MHz
        ; ((OSCCON) and 0FFh), 1, a:((OSCCON) and 0FFh), 0, a = 10 -> reloj interno
        ; Valor: 0b01100010
        MOVLW 0b01100010
        MOVWF OSCCON, a

    ; LED ((PORTD) and 0FFh), 0, a salida, apagado
    BCF TRISD, 0, a
        BCF LATD, 0, a

    ; Botón ((PORTB) and 0FFh), 0, a/((PORTB) and 0FFh), 0, a como entrada
        BSF TRISB, 0, a

    ; Pull-ups internos PORTB deshabilitados (((INTCON2) and 0FFh), 7, a=1)
        ; (Con botón a VDD + pull-down externo NO se usan pull-ups)
        BSF INTCON2, 7, a

    ; IINT0 por flanco de BAJADA (((INTCON2) and 0FFh), 6, a=0) porque botón va a GND (1->0)
        BCF INTCON2, 6, a

        ; Habilitar ((PORTB) and 0FFh), 0, a + ((INTCON) and 0FFh), 7, a
        BCF INTCON, 1, a ; ((INTCON) and 0FFh), 1, a = 0
