...
# 13 "Ejercicio3_Led_Sequence.asm" 2
...
INIT:
    ; OSCCON:
    ; ((OSCCON) and 0FFh), 6, a:((OSCCON) and 0FFh), 4, a = 110 -> 8 MHz
    ; ((OSCCON) and 0FFh), 1, a:((OSCCON) and 0FFh), 0, a = 10 -> reloj interno explícito
    ; Valor: 0b01100010
    MOVLW 0b01100010
    MOVWF OSCCON, a
    ...
    BSF TRISB, 0, a ; ((PORTB) and 0FFh), 0, a = entrada (botón secuencia)
    BSF TRISB, 1, a ; ((PORTB) and 0FFh), 1, a = entrada (botón velocidad)

    ; Al presionarse el botón va de 5V a GND
    BCF INTCON2, 6, a ; pone en 0 el bit 6 (((INTCON2) and 0FFh), 6, a)
    BCF INTCON2, 5, a ; pone en 0 el bit 5 (((INTCON2) and 0FFh), 5, a)

    ; Limpiar banderas
    BCF INTCON, 1, a ; limpia bandera de ((PORTB) and 0FFh), 0, a
    BCF INTCON3, 0, a ; limpia bandera de ((PORTB) and 0FFh), 1, a

    ; Habilitamos interrupciones
    BSF INTCON, 4, a ; =1, habilita ((PORTB) and 0FFh), 0, a
    BSF INTCON3, 3, a ; =1, habilita ((PORTB) and 0FFh), 1, a
    BSF INTCON, 7, a ; ((INTCON) and 0FFh), 7, a=1, habilita interrupciones globales
...
SECUENCIA_0: ; Desplazamiento izq ? der
    MOVLW 0x01
    MOVWF LATD, a ; enciende ((PORTD) and 0FFh), 0, a
    CALL DELAY
    MOVLW 0x02
    MOVWF LATD, a ; enciende ((PORTD) and 0FFh), 1, a
    CALL DELAY
    MOVLW 0x04
    MOVWF LATD, a ; enciende ((PORTD) and 0FFh), 2, a
    CALL DELAY
    MOVLW 0x08
    MOVWF LATD, a ; enciende ((PORTD) and 0FFh), 3, a
    CALL DELAY
    GOTO MAIN
...
SECUENCIA_2: ; Relleno progresivo
    MOVLW 0x01
    MOVWF LATD, a ; ((PORTD) and 0FFh), 0, a
    CALL DELAY
    MOVLW 0x03
    MOVWF LATD, a ; ((PORTD) and 0FFh), 0, a+((PORTD) and 0FFh), 1, a
    CALL DELAY
    MOVLW 0x07
    MOVWF LATD, a ; ((PORTD) and 0FFh), 0, a+((PORTD) and 0FFh), 1, a+((PORTD) and 0FFh), 2, a
    CALL DELAY
    MOVLW 0x0F
    MOVWF LATD, a ; todos
    CALL DELAY
...
SECUENCIA_3: ; Mitad interior vs esquinas
    MOVLW 0x06
    MOVWF LATD, a ; ((PORTD) and 0FFh), 1, a y ((PORTD) and 0FFh), 2, a (mitad)
    CALL DELAY
    MOVLW 0x09
    MOVWF LATD, a ; ((PORTD) and 0FFh), 0, a y ((PORTD) and 0FFh), 3, a (esquinas)
    CALL DELAY
    GOTO MAIN
...
;=========================================================================
ISR:
    BTFSC INTCON, 1, a ; ¿((INTCON) and 0FFh), 1, a=1? (botón secuencia)
    CALL PROC_INT0

    BTFSC INTCON3, 0, a ; ¿((INTCON3) and 0FFh), 0, a=1? (botón velocidad)
    CALL PROC_INT1

    RETFIE ; regresa y reactiva interrupciones (((INTCON) and 0FFh), 7, a=1)
                                ; (si usaramos RETURN las interrupciones quedarian apagadas para siempre)

; --- ((PORTB) and 0FFh), 0, a: cambio de secuencia ---
PROC_INT0:
    BCF INTCON, 1, a ; limpia bandera ((INTCON) and 0FFh), 1, a
    CALL DEBOUNCE ; espera antirrebote
    BTFSC PORTB, 0, a ; ¿((PORTB) and 0FFh), 0, a sigue en 0? (presionado)
...
; --- ((PORTB) and 0FFh), 1, a: cambio de velocidad ---
PROC_INT1:
    BCF INTCON3, 0, a ; limpia bandera ((INTCON3) and 0FFh), 0, a
    CALL DEBOUNCE ; espera antirrebote
    BTFSC PORTB, 1, a ; ¿((PORTB) and 0FFh), 1, a sigue en 0? (presionado)
    RETURN ; no, fue rebote, ignora
