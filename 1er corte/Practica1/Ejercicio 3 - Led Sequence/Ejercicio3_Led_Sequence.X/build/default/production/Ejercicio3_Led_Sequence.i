...
;=========================================================
; PIC18F4550 - 4 Secuencias de LEDs con control de velocidad
; Materia: Microcontroladores
; Descripcion:
; - 4 LEDs en RD0-RD3
; - Boton secuencia en RB0/INT0 (interrupcion, flanco bajada)
; - Boton velocidad en RB1 (polling, flanco bajada)
; - 3 velocidades: Lenta(500ms), Normal(250ms), Rapida(125ms)
; - 4 secuencias distintas de patrones LED
; Oscilador: Interno 8MHz
; Ensamblador: MPLAB X IDE
;
; Calculo Timer0 para 125ms base:
; 8MHz / 4 = 2MHz -> periodo = 0.5us
; 0.5us x 256 (preescaler 1:256) = 128us por tick
; 125ms / 128us = 976.5 ticks
; 65536 - 977 = 64559 = 0xFC6F
; Llamando RETARDO_BASE N veces:
; x1 = 125ms (Rapida)
; x2 = 250ms (Normal)
; x4 = 500ms (Lenta)
;=========================================================
...
# 1 "C:/Program Files/Microchip/MPLABX/v6.30/packs/Microchip/PIC18Fxxxx_DFP/1.7.171/xc8\\pic\\include/pic18.inc" 1 3





# 1 "C:/Program Files/Microchip/MPLABX/v6.30/packs/Microchip/PIC18Fxxxx_DFP/1.7.171/xc8\\pic\\include/pic18_chip_select.inc" 1 3
# 339 "C:/Program Files/Microchip/MPLABX/v6.30/packs/Microchip/PIC18Fxxxx_DFP/1.7.171/xc8\\pic\\include/pic18_chip_select.inc" 3
# 1 "C:/Program Files/Microchip/MPLABX/v6.30/packs/Microchip/PIC18Fxxxx_DFP/1.7.171/xc8\\pic\\include\\proc/pic18f4550.inc" 1 3
# 47 "C:/Program Files/Microchip/MPLABX/v6.30/packs/Microchip/PIC18Fxxxx_DFP/1.7.171/xc8\\pic\\include\\proc/pic18f4550.inc" 3
SPPDATA equ 0F62h
...
TOSU equ 0FFFh
# 6181 "C:/Program Files/Microchip/MPLABX/v6.30/packs/Microchip/PIC18Fxxxx_DFP/1.7.171/xc8\\pic\\include\\proc/pic18f4550.inc" 3
...
# 340 "C:/Program Files/Microchip/MPLABX/v6.30/packs/Microchip/PIC18Fxxxx_DFP/1.7.171/xc8\\pic\\include/pic18_chip_select.inc" 2 3
# 7 "C:/Program Files/Microchip/MPLABX/v6.30/packs/Microchip/PIC18Fxxxx_DFP/1.7.171/xc8\\pic\\include/pic18.inc" 2 3
...
# 24 "Ejercicio3_Led_Sequence.asm" 2
...
SEQ: DS 1 ; Secuencia actual (1-4)
VEL: DS 1 ; Velocidad actual (1=Rapida, 2=Normal, 4=Lenta)
PASO: DS 1 ; Paso actual dentro de la secuencia (0-N)
VCONT: DS 1 ; Contador de llamadas al retardo base
...
INICIO:
    ; Oscilador interno 8MHz
    ; ((OSCCON) and 0FFh), 6, a:((OSCCON) and 0FFh), 4, a = 111 -> 8MHz, SCS = 10 -> interno
    MOVLW 0b01110010
    MOVWF OSCCON, a

    ; Timer0: apagado, 16bits, interno, preescaler 1:256
    ; T0CON = 0b00000111
    MOVLW 0b00000111
    MOVWF T0CON, a

    ; PORTD: ((PORTD) and 0FFh), 0, a-((PORTD) and 0FFh), 3, a como salidas, todos apagados
    MOVLW 0b11110000 ; bit0-3 salida (0), bit4-7 entrada (1)
    MOVWF TRISD, a
    CLRF LATD, a ; Todos los LEDs apagados

    ; PORTB: ((PORTB) and 0FFh), 0, a y ((PORTB) and 0FFh), 1, a como entradas
    BSF TRISB, 0, a ; ((PORTB) and 0FFh), 0, a entrada (boton secuencia)
    BSF TRISB, 1, a ; ((PORTB) and 0FFh), 1, a entrada (boton velocidad)

    ; Pull-ups internos PORTB deshabilitados (usamos pull-ups externos)
    BSF INTCON2, 7, a ; ((INTCON2) and 0FFh), 7, a = 1 -> deshabilitados

    ; ((PORTB) and 0FFh), 0, a: flanco de SUBIDA (botones a VDD, pin sube de 0->1 al presionar)
    BSF INTCON2, 6, a ; ((INTCON2) and 0FFh), 6, a = 1 -> flanco subida

    ; Habilitar ((PORTB) and 0FFh), 0, a
    BCF INTCON, 1, a ; Limpiar bandera ((INTCON) and 0FFh), 1, a
    BSF INTCON, 4, a ; ((INTCON) and 0FFh), 4, a = 1
    BSF INTCON, 7, a ; ((INTCON) and 0FFh), 7, a = 1

    ; Variables iniciales
    MOVLW 1
    MOVWF SEQ, a ; Arranca en secuencia 1
    MOVLW 2
    MOVWF VEL, a ; Arranca en velocidad Normal (x2 = 250ms)
    CLRF PASO, a ; Paso 0
...
;=========================================================
MAIN_LOOP:
    ; Revisar boton velocidad (polling ((PORTB) and 0FFh), 1, a)
    BTFSC PORTB, 1, a ; Si ((PORTB) and 0FFh), 1, a=0 (no presionado) salta
    CALL CAMBIAR_VEL ; Si ((PORTB) and 0FFh), 1, a=1 (presionado) cambia velocidad

    ; Ejecutar paso de la secuencia actual
    MOVF SEQ, W, a
    XORLW 1
    BZ EXEC_SEQ1 ; Si SEQ==1 ir a secuencia 1
    ...
    GOTO EXEC_SEQ4 ; Si no es 1,2,3 entonces es 4

VOLVER_LOOP:
    ; Retardo segun velocidad actual
    MOVF VEL, W, a
    MOVWF VCONT, a ; VCONT = numero de veces a llamar RETARDO_BASE
...
;=========================================================
; SECUENCIA 1 - Desplazamiento derecha a izquierda
; Pasos: 0001 -> 0010 -> 0100 -> 1000 -> repite
;=========================================================
EXEC_SEQ1:
    MOVF PASO, W, a
    XORLW 0
...
SEQ1_P3:
    MOVLW 0b00001000
    MOVWF LATD, a
    CLRF PASO, a ; Reinicia al paso 0 en el siguiente ciclo
    GOTO VOLVER_LOOP
...
;=========================================================
; SECUENCIA 2 - Llenado progresivo
; Pasos: 0001 -> 0011 -> 0111 -> 1111 -> 0000 -> repite
;=========================================================
EXEC_SEQ2:
    MOVF PASO, W, a
    XORLW 0
...
;=========================================================
; SECUENCIA 3 - Parpadeo alterno (par/impar)
; Pasos: 1010 -> 0101 -> repite
;=========================================================
EXEC_SEQ3:
    MOVF PASO, W, a
    XORLW 0
...
;=========================================================
; SECUENCIA 4 - Rebote (izquierda y vuelta)
; Pasos: 0001->0010->0100->1000->0100->0010->repite
;=========================================================
EXEC_SEQ4:
    MOVF PASO, W, a
    XORLW 0
...
;=========================================================
; CAMBIAR_VEL - Cicla entre velocidades 1, 2, 4
; Rapida(1) -> Normal(2) -> Lenta(4) -> Rapida(1)
;=========================================================
CAMBIAR_VEL:
    ; Esperar que suelten el boton (antirebote simple)
    CALL RETARDO_BASE

    MOVF VEL, W, a
    XORLW 1
    BZ VEL_A_NORMAL ; Si era Rapida -> pasar a Normal

    MOVF VEL, W, a
    XORLW 2
    BZ VEL_A_LENTA ; Si era Normal -> pasar a Lenta

    ; Si era Lenta (4) -> pasar a Rapida
    MOVLW 1
    MOVWF VEL, a
    RETURN
...
;=========================================================
; RETARDO_BASE - Retardo de ~125ms usando Timer0
; Timer arranca en 64559 (0xFC6F) hasta desborde 65536
; Ticks necesarios: 977
; 977 x 128us = 125.056ms
;=========================================================
RETARDO_BASE:
    BCF T0CON, 7, a ; Apagar timer
    MOVLW 0xFC
    MOVWF TMR0H, a ; Byte alto de 64559
    MOVLW 0x6F
    MOVWF TMR0L, a ; Byte bajo de 64559
    BCF INTCON, 2, a ; Limpiar bandera ((INTCON) and 0FFh), 2, a
    BSF T0CON, 7, a ; Encender timer

ESPERAR:
    BTFSS INTCON, 2, a ; Esperar desborde
    GOTO ESPERAR
...
;=========================================================
; ISR - Interrupcion por ((PORTB) and 0FFh), 0, a (boton secuencia ((PORTB) and 0FFh), 0, a)
;=========================================================
ISR:
    BTFSS INTCON, 1, a ; Verificar que sea ((INTCON) and 0FFh), 1, a
    RETFIE ; No era ((PORTB) and 0FFh), 0, a, salir

    BCF INTCON, 1, a ; Limpiar bandera ((INTCON) and 0FFh), 1, a

    ; Avanzar secuencia: SEQ = SEQ + 1, si >4 volver a 1
    INCF SEQ, F, a
    MOVF SEQ, W, a
    XORLW 5
    BNZ ISR_RESET_PASO ; Si SEQ != 5, ok
    MOVLW 1 ; Si SEQ == 5, volver a 1
    MOVWF SEQ, a

ISR_RESET_PASO:
    CLRF PASO, a ; Reiniciar paso al cambiar de secuencia
