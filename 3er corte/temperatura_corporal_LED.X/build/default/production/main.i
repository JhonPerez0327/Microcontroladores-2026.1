...
unsigned char OW_Reset(void) {
    ...
    do { TRISBbits.TRISB0 = 0; LATBbits.LATB0 = 0; } while(0);
    _delay((unsigned long)((480)*(8000000/4000000.0)));

    do { TRISBbits.TRISB0 = 1; } while(0);
    _delay((unsigned long)((70)*(8000000/4000000.0)));

    presencia = !(PORTBbits.RB0);
    _delay((unsigned long)((410)*(8000000/4000000.0)));
...
void OW_WriteByte(unsigned char dato) {
    ...
    for(i = 0; i < 8; i++) {
        if(dato & 0x01) {
            ...
            do { TRISBbits.TRISB0 = 1; } while(0);
            _delay((unsigned long)((54)*(8000000/4000000.0)));
        } else {
...
void DS18B20_LeerTemperatura(unsigned char *entero, unsigned char *decimal) {
    ...
    byte_bajo = OW_ReadByte();
    byte_alto = OW_ReadByte();
...
void main(void) {
    ...
    while(1) {
        ...
        if(temp_ent > 36 && temp_ent < 37) {

            en_rango = 1;
        } else if(temp_ent == 36 && temp_dec >= 0) {

            en_rango = 1;
        } else if(temp_ent == 37 && temp_dec <= 5) {

            en_rango = 1;
        }

        LATDbits.LATD0 = en_rango;
